...
export function createStreamButton(stream: TwitchStream): ActionRowBuilder<ButtonBuilder> {
	const button = new ButtonBuilder().setLabel('Watch Stream').setStyle(ButtonStyle.Link).setURL(`https://twitch.tv/${stream.user_login}`)

	return new ActionRowBuilder<ButtonBuilder>().addComponents(button)
}

export function createVODButton(vod: TwitchVOD): ActionRowBuilder<ButtonBuilder> {
	const button = new ButtonBuilder().setLabel('Watch VOD').setStyle(ButtonStyle.Link).setURL(vod.url)
