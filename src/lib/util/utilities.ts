...
export async function getInput(chnl: SendableChannels, options: Options): Promise<string[]> {
	const filter = options.userId
		? (m: Message) => m.author.id === options.userId && m.channel.id === chnl.id
		: (m: Message) => m.channel.id === chnl.id
	...
	try {
		...
		if (options.deleteAfter) {
			collected.forEach((m) => {
				chnl.messages.cache.delete(m.id)
			})
...
export async function timerMessage(m: Message, delay: number = ms('5s')) {
	return new Promise((resolve) => setTimeout(resolve, delay)).then(() => m?.delete())
}
