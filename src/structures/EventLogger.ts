...
export default class EventLogger {
    ...
    async getLatestEventLogs(guild: Guild, limit: number = 10): Promise<IEventLog[]> {
        return EventLog.find({ guildId: guild.id })
            .sort({ caseId: -1 })
            .limit(limit)
    }
    ...
    private async createLogEntry(
        ...
        const embedDescription = data.content || data.beforeContent || data.afterContent ?
            (data.beforeContent && data.afterContent ?
                `**Before:** ${data.beforeContent}\n**After:** ${data.afterContent}` :
                data.content) : undefined

        const embed = embedDescription ? new EmbedBuilder()
            .setColor(color)
