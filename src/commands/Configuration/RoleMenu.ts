...
@ApplyOptions<Subcommand.Options>({
	...
	detailedDescription:
		'Create interactive menus where users can react with emojis to get roles. Uses an interactive setup process to assign emojis to roles.',
	examples: [
...
export class RoleMenuCommand extends Subcommand {
	...
	public async messageCreate(message: Message, args: Args) {
		...
		return (await this.client.tasks.get('rolemenuwizard')?.exec(message, groupName, targetChannel, targetMessageId)) || false
	}
			...
			try {
				...
				if (removed) {
					try {
						...
						if (channel?.isTextBased()) {
							const menuMessage = await channel.messages.fetch(menu.messageId)
							const reaction = menuMessage.reactions.cache.find((r) => r.emoji.toString() === emoji || r.emoji.name === emoji)
							if (reaction) {
	...
	public async messageRefresh(message: Message, args: Args) {
		...
		try {
			...
			const embed = new EmbedBuilder().setTitle(menu.title).setDescription(menu.description).setColor(Colors.Blurple)

			const rolesList = groupRoles
				.map((gr) => {
	...
	public async chatInputCreate(interaction: Subcommand.ChatInputCommandInteraction) {
		...
		if (groupRoles.length === 0) {
			return interaction.reply({
				content: `❌ Role group **${groupName}** has no roles. Add roles first with \`/rolegroup addrole\`.`,
				ephemeral: true
			})
		}
	...
	public async chatInputRefresh(interaction: Subcommand.ChatInputCommandInteraction) {
		return this.messageRefresh(
			interaction as any,
			{ pickResult: () => ({ isOk: () => true, unwrap: () => interaction.options.getString('group', true) }) } as any
		)
	}
