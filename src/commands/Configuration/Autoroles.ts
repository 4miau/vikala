...
@ApplyOptions<Subcommand.Options>({
	...
	detailedDescription:
		'Comprehensive automatic role management system that assigns roles based on member join events, server boosts, or time-based delays. Supports priorities, conditional assignments, and flexible rule management for streamlined server moderation.',
...
export class AutorolesCommand extends Subcommand {
	...
	public async messageList(message: Message) {
		...
		const embed = new EmbedBuilder().setTitle('🤖 Autorole Rules').setColor(Colors.Blurple).setDescription(`Found ${rules.length} autorole rule(s)`)
	...
	public async messageAdd(message: Message, args: Args) {
		...
		if (!type || !['join', 'time', 'boost'].includes(type.toLowerCase()))
			return message.channel.send('❌ Please specify a valid type: `join`, `time`, or `boost`\nExample: `autoroles add join @Member`')
	...
	public async chatInputToggle(interaction: Subcommand.ChatInputCommandInteraction) {
		...
		if (updated)
			return interaction.reply({ content: `✅ ${status.charAt(0).toUpperCase() + status.slice(1)} autorole rule for ${role}`, flags: ['Ephemeral'] })
		return interaction.reply({ content: `❌ No autorole rule found for ${role}`, flags: ['Ephemeral'] })
	}
