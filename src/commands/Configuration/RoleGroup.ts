...
@ApplyOptions<Subcommand.Options>({
	...
	detailedDescription:
		'Create and manage role groups that define how roles can be assigned together. Groups can have different modes: single (one role only), multiple (any number), or limited (min/max constraints).',
	examples: [
		...
		{
			example: 'rg settings colors --required @Verified --remove @Uncolored',
			description: 'Configure group to require @Verified role and remove @Uncolored when assigning.'
		},
...
export class RoleGroupCommand extends Subcommand {
	...
	public async chatInputInfo(interaction: Subcommand.ChatInputCommandInteraction) {
		return this.messageInfo(
			interaction as any,
			{ pickResult: () => ({ isOk: () => true, unwrap: () => interaction.options.getString('name', true) }) } as any
		)
	}
