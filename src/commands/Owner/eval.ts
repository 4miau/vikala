...
import ms from 'ms'
import { OBJECT_INSPECTION_DEPTH_MAX, OBJECT_INSPECTION_DEPTH_MIN } from '../../lib/util/constants'
...
@ApplyOptions<Command.Options>({
	...
	detailedDescription:
		'Evaluates JavaScript code with timeout support, context injection, and smart output handling. ' +
...
export class Eval extends Command {
	...
	private cleanOutput(text: string): string {
		return text.replace(new RegExp(this.client.token || 'undefined', 'gi'), '[TOKEN]').replace(/[\u200B-\u200D\uFEFF]/g, '')
	}
			...
			builder
				...
				.addIntegerOption((option) =>
					option
						...
						.setRequired(false)
						.setMinValue(OBJECT_INSPECTION_DEPTH_MIN)
						.setMaxValue(OBJECT_INSPECTION_DEPTH_MAX)
				)
		)
	}
}
