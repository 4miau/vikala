...
@ApplyOptions<Subcommand.Options>({
	...
	detailedDescription:
		'Search for games in the GAMES LIBRARY or GAMES ARCHIVE, and transfer games from library to archive. ' +
...
export class VHSGame extends Subcommand {
	...
	private async handleGameView(
		...
		try {
			if (isNumeric || isLatest) {
				const gameIndex = isLatest ? await this.client.sheets.getLatestGameIndex(sheetTitle) : parseInt(query)
	...
	private async handleGameTransfer(
		gameId: number,
		sourceSheet: string,
		context: {
