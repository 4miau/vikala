...
@ApplyOptions<Subcommand.Options>({
	...
	examples: [
		{
			example: 'setpresence set Playing online "with Discord APIs"',
			description: 'Sets the bot presence to "Playing with Discord APIs" with an online status.'
		},
		...
		{
			example: 'setpresence add "chilling" Watching idle',
			description: 'Adds a new presence "Watching chilling" with idle status to the cycle list.'
		},
		{
			example: 'setpresence add "live coding" Streaming dnd "https://twitch.tv/4miau"',
			description: 'Adds a streaming presence with URL to the cycle list.'
		},
...
export class SetPresence extends Subcommand {
	...
	private buildPresenceListEmbed(presences: any[]) {
		const embed = new EmbedBuilder().setTitle('🎭 Presence Cycle List').setColor(Colors.Purple).setTimestamp()

		if (presences.length === 0) {
			embed.setDescription('No presences in cycle list.')
		} else {
			const list = presences.map((p, i) => `${i + 1}. **${ActivityType[p.type]}** *${p.name}* (${p.status})`).join('\n')
		...
		registry.registerChatInputCommand((builder) =>
			builder
				...
				.addSubcommand((subcommand) =>
					subcommand
						.setName('list')
						.setDescription('List all presences in cycle')
				)
				.addSubcommand((subcommand) =>
					subcommand
						.setName('interval')
				...
				.addSubcommand((subcommand) =>
					subcommand
						.setName('status')
						.setDescription('Show current presence manager status')
				)
		)
	}
}
