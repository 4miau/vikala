...
@ApplyOptions<Subcommand.Options>({
    ...
    detailedDescription: 'Add, remove, and manage Twitch streamers to get notified when they go live. When using add, only the streamer name is required. ' +
    ...
    examples: [
        ...
        { example: 'twitch add 4miau #general Welcome to the stream!', description: 'Adds 4miau as a tracked streamer and posts a message in #general when they go live.' },
...
export class Twitch extends Subcommand {
    ...
    private async handleEmbed(name: string, guild: any, state: boolean, sendFn: (content: any) => Promise<any>) {
        ...
        const embed = new EmbedBuilder()
            .setTitle(`Embed preference updated for ${name}`)
