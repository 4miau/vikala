...
export class LevelCommand extends Subcommand {
		...
		const embed = new EmbedBuilder()
			...
			.addFields(
				...
				{ name: 'Messages Sent', value: userLevel.messageCount.toLocaleString(), inline: true },
				{
					...
					value: `${progressBar}\n${progressXP.toLocaleString()}/${totalNeededForNext.toLocaleString()} XP (${progressPercentage}%)`,
					inline: false
				}
			)
			.setTimestamp()
	...
	private async createLeaderboardEmbed(leaderboard: any[]): Promise<EmbedBuilder> {
		const embed = new EmbedBuilder().setTitle('🏆 Level Leaderboard').setColor(Colors.Golden).setTimestamp()
			...
			const embed = new EmbedBuilder()
				...
				.addFields([
					...
					{ name: 'Level Up Channel', value: config.levelUpChannel ? `<#${config.levelUpChannel}>` : 'Same channel', inline: true },
					{
						...
						value: config.enabledChannels?.length ? config.enabledChannels.map((id: string) => `<#${id}>`).join(', ') : 'All channels',
						inline: false
					},
					{
						name: 'Disabled Channels',
						value: config.disabledChannels?.length ? config.disabledChannels.map((id: string) => `<#${id}>`).join(', ') : 'None',
						inline: false
					},
					{
						name: 'Role Multipliers',
						value:
							config.multiplierRoles && Object.keys(config.multiplierRoles).length
								? Object.entries(config.multiplierRoles)
