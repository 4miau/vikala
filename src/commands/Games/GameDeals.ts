...
@ApplyOptions<Command.Options>({
    ...
    detailedDescription: 'Fetches the latest game deals for a specified Steam game ID from gg.deals, including prices from various keyshops. The default region is US, but you can specify other regions like EU for prices in EUR.',
...
export class GameDeals extends Command {
    ...
    private buildGameDealsEmbed(game: GGDealsGameResponse) {
        return new EmbedBuilder()
            .setAuthor({ name: game.title, url: game.url })
            .setDescription(`
