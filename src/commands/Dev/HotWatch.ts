        ...
        try {
            switch (action.toLowerCase()) {
                case 'status': {
                    ...
                    embed.setTitle('🔍 Hot Reload Watcher Status')
                        .addFields(
                            {
                                name: 'Status',
                ...
                case 'start': {
                    if (this.client.hotReloadWatcher.isRunning()) {
                        embed.setTitle('⚠️ Already Running')
                            .setDescription('Hot reload watcher is already active.')
                            .setColor(Colors.HighlightYellow)
                    } else {
                        this.client.hotReloadWatcher.start()
                        embed.setTitle('✅ Watcher Started')
                ...
                case 'stop': {
                    if (!this.client.hotReloadWatcher.isRunning()) {
                        embed.setTitle('⚠️ Already Stopped')
                            .setDescription('Hot reload watcher is not currently running.')
                            .setColor(Colors.HighlightYellow)
                    } else {
                        await this.client.hotReloadWatcher.stop()
                        embed.setTitle('🛑 Watcher Stopped')
                ...
                case 'restart': {
                    ...
                    embed.setTitle('🔄 Watcher Restarted')
                        .setDescription('Hot reload file watcher has been restarted.')
                        .setColor(Colors.Active)
                    break
                }
                ...
                case 'blacklist': {
                    ...
                    embed.setTitle('🚫 Blacklisted Structures')
