...
@ApplyOptions<Command.Options>({
    ...
    examples: [
        { example: 'dismantle ping command', description: 'Dismantles the command named ping and unloads it from the bot.' },
    ],
...
export class Dismantle extends Command {
    ...
    public async messageRun(message: Message, args: Args) {
        ...
        const type: ModuleType = await args.pickResult('enum', { enum: ['command', 'listener', 'precondition', 'task', 'arguments'] }).then(res => res.isOk() ? res.unwrap() : null) as ModuleType
        ...
        registry.registerChatInputCommand((builder) =>
            builder
                ...
                .addStringOption((option) =>
                    option
                        .setName('file')
                        .setDescription('The file/module to dismantle.')
                        .setRequired(true)
                )
                .addStringOption((option) =>
                    option
