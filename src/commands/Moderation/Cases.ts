...
@ApplyOptions<Subcommand.Options>({
	...
	detailedDescription:
		'Comprehensive case management system for viewing moderation actions and server event logs with filtering and search capabilities.',
...
export class CasesCommand extends Subcommand {
	...
	public async messageRecent(message: Message, args: Args) {
		...
		const allCases = [...modCases, ...eventLogs].sort((a, b) => b.caseId - a.caseId)
	...
	public async messageView(message: Message, args: Args) {
		...
		const [modCase, eventLog] = await Promise.all([this.client.cases.getCase(message.guild, id), this.client.events.getEventLog(message.guild, id)])

		if (!modCase && !eventLog) {
			return message.channel.send(`❌ No case or event found with ID ${id}.`)
		}

		const embed = new EmbedBuilder().setColor(modCase ? Colors.Orange : Colors.Blurple)
	...
	public async messageUser(message: Message, args: Args) {
		...
		const allCases = [...modCases, ...eventLogs].sort((a, b) => b.caseId - a.caseId).slice(0, 15)
	...
	private createPaginationRow(currentPage: number, totalPages: number): ActionRowBuilder<ButtonBuilder> {
		...
		const buttons = [
			...
			new ButtonBuilder()
				...
				.setStyle(ButtonStyle.Secondary)
				.setDisabled(currentPage === 1),
			new ButtonBuilder().setCustomId('cases-current').setLabel(`${currentPage}/${totalPages}`).setStyle(ButtonStyle.Primary).setDisabled(true),
			new ButtonBuilder()
