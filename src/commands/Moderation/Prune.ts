...
@ApplyOptions<Subcommand.Options>({
	...
	detailedDescription:
		'Prune messages in the server based on various filters. Default is 50. You can use options `--amount` (or `--a` for short) ' +
...
export class Prune extends Subcommand {
	...
	public async pruneImagesMsg(message: Message, args: Args) {
		...
		const amount = args
			...
			.unwrapOrElse(() => 50)
		const messages = await message.channel.messages.fetch({ limit: amount })
		const deleted = await this.executeMessagePrune(
			messages,
			(m) => m.attachments.some((att) => att.contentType?.startsWith('image/')) || /\.(jpg|jpeg|png|gif|webp|bmp|svg)$/i.test(m.content)
		)
	...
	public async pruneImagesInput(interaction: Subcommand.ChatInputCommandInteraction) {
		...
		const deleted = messages
			? await this.executeMessagePrune(
					messages,
					(m) => m.attachments.some((att) => att.contentType?.startsWith('image/')) || /\.(jpg|jpeg|png|gif|webp|bmp|svg)$/i.test(m.content)
				)
			: 0
	...
	public async pruneEmojiInput(interaction: Subcommand.ChatInputCommandInteraction) {
		...
		const messages = await interaction.channel?.messages.fetch({ limit: amount })
		const deleted = messages
			? await this.executeMessagePrune(messages, (m) => {
					const hasCustomEmoji = /<a?:[a-zA-Z0-9_]+:[0-9]+>/g.test(m.content)
	...
	private async handlePruneResult(deleted: number, context: Message | Subcommand.ChatInputCommandInteraction, messageType = 'messages') {
		...
		if (context instanceof Message) {
			return context.channel.isSendable()
				? context.channel.send({ content: successMsg }).then((m: Message) => setTimeout(() => m?.delete(), 5000))
				: null
		}
