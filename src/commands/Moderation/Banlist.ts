...
@ApplyOptions<Command.Options>({
	...
	detailedDescription:
		'View all currently banned users with their usernames, IDs, and ban reasons. Navigate through pages using interactive buttons.',
	usage: 'bans',
	examples: [{ example: 'bans', description: 'Display the ban list with interactive pagination.' }],
	runIn: 'GUILD_ANY',
	...
	private createPaginationRow(currentPage: number, totalPages: number): ActionRowBuilder<ButtonBuilder> {
		...
		const buttons = [
			...
			new ButtonBuilder()
				...
				.setStyle(ButtonStyle.Secondary)
				.setDisabled(currentPage === 1),
			new ButtonBuilder().setCustomId('bans-current').setLabel(`${currentPage}/${totalPages}`).setStyle(ButtonStyle.Primary).setDisabled(true),
			new ButtonBuilder()
