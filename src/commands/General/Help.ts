...
@ApplyOptions<Command.Options>({
	...
	detailedDescription:
		'Displays help information about the bot commands. You can also get detailed information about a specific command by providing its name as an argument.',
	usage: 'help [command]',
	examples: [
...
export class Help extends Command {
	...
	private createCommandHelpRow(currentPage: number, totalPages: number): ActionRowBuilder<ButtonBuilder> {
		...
		const buttons = [
			...
			new ButtonBuilder().setCustomId('cmd-help-current').setLabel(`${currentPage}/${totalPages}`).setStyle(ButtonStyle.Primary).setDisabled(true),
			new ButtonBuilder()
	...
	private createPaginationRow(currentPage: number, totalPages: number, isCommandSpecific: boolean = false): ActionRowBuilder<ButtonBuilder> {
		...
		const buttons = [
			...
			new ButtonBuilder()
				...
				.setStyle(ButtonStyle.Secondary)
				.setDisabled(currentPage === 1),
			new ButtonBuilder().setCustomId('help-current').setLabel(`${currentPage}/${totalPages}`).setStyle(ButtonStyle.Primary).setDisabled(true),
			new ButtonBuilder()
