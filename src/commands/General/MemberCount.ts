...
export class MemberCount extends Command {
	...
	private buildMemberCountEmbed(guild: Guild) {
		return new EmbedBuilder()
			...
			.addFields(
				...
				{
					name: 'Online',
					value: `${guild.members.cache.filter((m) => m.presence?.status !== 'invisible' && m.presence?.status !== 'offline').size}`,
					inline: true
				},
	...
	public override registerApplicationCommands(registry: Command.Registry) {
		registry.registerChatInputCommand((builder) => builder.setName('membercount').setDescription('Get the member count of the server'))
	}
}
